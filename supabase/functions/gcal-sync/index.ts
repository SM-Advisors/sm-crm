...
Deno.serve(async (req) => {
  ...
  try {
    switch (action) {
      ...
      // ── lookup-contacts-by-emails ──
      // Takes an array of attendee emails, returns matched contacts
      case "lookup-contacts-by-emails": {
        const emails = body.emails as string[];
        if (!emails || !Array.isArray(emails) || emails.length === 0) {
          return err("emails array required");
        }
        ...
        // Return as a map of email -> { contact_id, company_id }
        const matches: Record<string, { contact_id: string; company_id: string | null }> = {};
        if (data) {
          for (const row of data) {
            if (row.email) {
              matches[row.email.toLowerCase()] = {
                contact_id: row.id,
        ...
        return json({ matches, matched_count: Object.keys(matches).length });
      }
