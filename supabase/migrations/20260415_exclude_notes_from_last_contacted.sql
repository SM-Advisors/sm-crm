-- Exclude 'note' interaction type from last_contacted_at calculations.
-- Notes are not real contact activity — they're internal annotations.

-- 1. Fix the INSERT trigger: skip notes and future-dated interactions
CREATE OR REPLACE FUNCTION update_last_contacted()
RETURNS TRIGGER AS $$
BEGIN
  -- Skip notes — they are not real contact activity
...
-- 2. Fix the DELETE trigger: exclude notes and future-dated interactions when recalculating
...
-- 3. Recalculate all contacts excluding notes and future dates
UPDATE contacts c
SET
...
-- Clear last_contacted_at for contacts with no qualifying interactions
UPDATE contacts c
SET
